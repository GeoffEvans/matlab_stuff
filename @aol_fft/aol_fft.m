classdef aol_fft
    ...
    methods
        ...
        function [propagated_wave, x, y] = calculate_psf(obj, sampled_wave_2d, space_width)
            ...
            propagated_wave = obj.propagate_wave(focal_plane_wave_2d, obj.z_list, space_width);
        end
        ...
        function propagated_wave = propagate_wave(obj, wave_2d, distance, space_width)
            ...
            %display(max(k_x(:))/k); display((k_y(2)-k_y(1))/k);
            k_z = sqrt(obj.k.^2 - k_x.^2 - k_y.^2);
            mask = ~(abs(imag(k_z)) > 0);
        ...
        function xy_xz_plot_psf(obj, propagated_wave_2d, x, y, z_plane_frac, x_plane_frac)
            figure(); 
            subplot(1,2,1)
            h = pcolor(x, y, abs(propagated_wave_2d(:,:,round(z_plane_frac*size(obj.z_list,1)))));
            set(h,'EdgeColor','none')
            axis equal

            subplot(1,2,2)
            [zz, xx] = meshgrid(obj.z_list, x(round(x_plane_frac*size(x,1)),:));
        ...
        function [sampled_wave, space_width] = get_sampled_wavefunction(obj, waves, t)     
            ...
            scale_factors = 2*pi / obj.k ./ (6e-3).^(1:5); % take aperture to be 12mm because of scaling by relay
            distances = [0, ones(1,numel(waves.aods)-1)*0.05];
            directions = linspace(0, 2 * pi, numel(waves.aods) + 1);
            T = obj.V * t;
            ...
            for n = 1:numel(waves.aods)
                phase = obj.k * (waves.aods{n} .* scale_factors);
                sampled_wave = obj.propagate_wave(sampled_wave, distances(n), space_width)...
            ...
            focal_length = inv(4 * phase(2) / obj.k)
