...
Rays(2,:,:,:)=Rays(1,:,:,:)+RayDirVecAfterAodN(1,:,:,:).*(Zp(2)-Zp(1));

for n=2:7
    l(n,:,:,:)=(abs(Rays(n,:,:,:))).*cos(angle(Rays(n,:,:,:))-thetan(n-1));
    ...
    RayDirVecAfterAodN(n,:,:,:)= RayDirVecAfterAodN(n-1,:,:,:)+DeflectOnAodN(n,:,:,:); % Ray direction vector justafter each Z plane
    Rays(n+1,:,:,:)=Rays(n,:,:,:)+RayDirVecAfterAodN(n,:,:,:).*(Zp(n+1)-Zp(n));
end
